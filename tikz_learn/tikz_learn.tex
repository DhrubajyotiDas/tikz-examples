...
% \tikzstyle{block} = [rectangle, draw, text width = 2 em, 
% 					text centered, fill = blue!20,
% 					minimum height = 2 em]
% \tikzstyle{line} = [draw, ->]
...
\path [line] (begin) edge node {$a$} (a);
\path [line] (a) edge node {$v$} (c);
\path [line] (c) edge node {$p$} (end);

% \node [block] (1/s1) {$\dfrac{1}{s}$};

% \node [block, right = of 1/s1] (1/s2) {$\dfrac{1}{s}$};
% \path [line] (1/s1) -- (1/s2);
\end{tikzpicture}
...
% \draw [fill = blue!20] (2, -0.5) rectangle (3, 0.5); 
% \node [center] at (2.5, 0) {$frac{1}{s}$};
% \draw [thick, ->] (3,0) to (6,0);
% \node [above] at (4.5, 0.1) {$v$};
% \draw [fill = blue!20] (6, -0.5) rectangle (7, 0.5);
% \draw [thick, ->] (7,0) to (9,0);
% \node [above] at (8, 0.1) {$p$};
% \end{tikzpicture}


% subsection simple_block_diagram (end)
